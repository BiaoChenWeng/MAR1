...
public:
	...
	std::pair<bool,T> kesimo(int k)const {
		return kesimoBusqueda(k, this->raiz);
	}

protected:

	std::pair<bool,T> kesimoBusqueda(int k ,Link r)const  {
		if (r == nullptr) {// no lo encontro, en este caso pasa por out of bound
			std::pair<bool, T>ans;
			ans.first = false;
			return ans;
		}
		else if (k < r->tam_i) {// si tiene mas hijo que k buscamos en la parte izq 
			return kesimoBusqueda(k, r->iz);
		}
		else if (k > r->tam_i) {// si k es meyor que los  hijos, le restamos los hijos izq que descartamos y buscamos en la der
			return kesimoBusqueda(k - r->tam_i, r->dr);
		}
		else {
			return {true,r->elem};// el numero de hijo es el mismo que k , es el elemento buscado
		}
	}
