...
public:
	...
	void buscarEntre(T menor, T mayor, std::vector<int>& v) {
		buscarE(menor, mayor, v, this->raiz);
...
protected:
	...
	void buscarE(const T& menor, const T& mayor, std::vector<T>& v, Link& r) {
		if (r != nullptr) {
			if (r->elem <= menor) {
				if (r->elem == menor)v.push_back(r->elem);
				buscarE(menor, mayor, v, r->dr);
				
			}
			else if (r->elem >= mayor) {
				buscarE(menor, mayor, v, r->iz);
				if (r->elem == mayor)v.push_back(r->elem);
			}
			else {
				buscarE(menor, mayor, v, r->iz);
				v.push_back(r->elem);
